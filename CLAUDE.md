...
- **기록 흐름 (2026-09-15 변경)**: 산행 시작·종료는 지도 홈의 버튼. 코스를 미리 고르지 않고 시작하며,
  종료 시 트랙을 모든 코스와 대조해 커버율이 가장 높은 코스를 기본 선택으로 제시 → 사용자가 한 번 확인(완주/일부/삭제).
  코스 화면의 [산행 시작]은 미리 고른 코스로 바로 확인하는 지름길. 코스 단위 판별이며 구간 단위 맵매칭은 v2
- **홈 구조 (2026-09-16 변경)**: 하단 탭 없음. 홈 = 지도 하나. 상단 검색바(탭하면 검색 화면) + 우상단 메뉴(내 기록·설정).
  하단은 드래그 가능한 바텀시트: 선택 없음 → 내 통계, 산 선택 → 산 요약·코스·지하철, 코스 선택 → 코스 설명.
  시트 위에 기록 버튼([기록 시작] → 기록 중 [휴식/다시 출발][정지]). 휴식은 UI 표시만, 위치 저장은 계속. 소요시간 = 점 기반 이동 시간. 기록 중 트랙은 메인 지도에 바로 그림.
  뒤로가기: 검색으로 고른 선택이 있으면 검색 화면으로 복귀, 없으면 앱 종료. 카메라 이동으로 근처 산이 자동 포커스되는 건 지도 강조만(시트 안 올림)
