# CLAUDE.md — 산책노트 (sannote)

## 프로젝트 개요
걸으면서 지도를 칠하는 기록 앱. 표시 이름 **산책노트** (코드명·번들 ID는 sannote 유지).
핵심 루프: 회색 밑그림(등산로망) 위를 걸으면 닿은 구간이 그 산의 색연필 색으로 칠해지고,
숨겨진 코스의 구간을 다 칠하면 "코스 획득" 연출과 함께 코스 카드가 열린다(리빌).
**앱 = 지도 + [기록 시작] 하나.** 검색·탐색 화면 없음. 메뉴엔 "내가 모은 산책로"와 설정만.
(2026-09-17 피벗. 이전 컨셉 "입구·코스 단위 큐레이션 정보 앱"의 데이터·도구는 그대로 쓰되, 정보는 발견 후에만 보인다)
...
- 코스는 자동 생성하지 않음. 운영자가 큐레이션 (courses.segment_ids 수동 입력). 코스는 **숨겨진 상태**로 시작해 구간을 다 칠하면 리빌
...
- **기록 흐름 (2026-09-17 피벗)**: 시작·종료는 지도 홈의 버튼만. 코스 선택 없음.
  종료 시 ① 트랙에 닿은 구간(40m 이내)을 **구간 단위로 칠함**(맵매칭, v1으로 당김) ② 어떤 코스의 구간이 전부(≥90%) 칠해졌으면
  그 코스를 **획득** → 획득 연출(지도에서 코스 모양이 튀어올라 회전·색 채움·파티클·손글씨 "코스 획득!") → 내가 모은 산책로에 카드 추가.
  사용자 확인은 "기록 저장/삭제" 한 번만. 자동 판정엔 여전히 수동 확인이 있으나, 완주 여부를 고르는 단계는 없다(칠해진 만큼이 결과)
- **홈 구조 (2026-09-17 피벗)**: 홈 = 지도 하나. 상단 좌측 손글씨 앱 이름, 우상단 메뉴(내가 모은 산책로·설정). 검색 없음.
  지도: 회색 밑그림(전체 등산로망) + 내가 칠한 구간(산별 색) + 산 이름 라벨(마커 없음, 탭 없음). 내 위치 기준.
  하단 시트: 접힘 = 한 줄 통계(칠한 길 km · 획득 코스 n), 펼침 = 통계 타일·스탬프·최근 산행. 시트 위 [기록 시작] → 기록 중 [휴식][정지].
  획득한 코스는 지도에서 탭하면 시트에 코스 설명(거리·시간·경유지·구조표지판)이 열림. 미획득 코스는 지도에 표시하지 않음.
- **지도는 항상 내 위치가 기준점**: 지도 홈 진입 시 위치 권한을 요청하고 내 위치로 카메라를 맞춘다(줌 12.5).
  권한 거부 시 7개 산군 개요 + 안내 문구. 위치 권한 요청은 지도 홈에서만 (다른 화면은 이미 허용된 경우만 읽음)
